  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://matty-ai-online-graphic-design-tool.onrender.com/api/users/login",
        formData,
...
const handleGoogleSignup = async () => {
  ...
  const top = window.screen.height / 2 - height / 2;
   ...
   window.open(
    "https://matty-ai-online-graphic-design-tool.onrender.com/api/users/google",
    "Google Signup",
    `width=${width},height=${height},top=${top},left=${left}`
  );

  window.addEventListener("message", (event) => {
    if (event.origin !== "https://matty-ai-online-graphic-design-tool.onrender.com") return;
